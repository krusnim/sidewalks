# Credit: https://gist.github.com/marmakoide/45d5389252683ae09c2df49d0548a627
...
def powerdiag(points, radii, area):

    S = numpy.array([q.coords[0] for q in points])
    R = numpy.array(radii)

    bound = area.length/2 # probably good enough?
    ...
    for segment_list in voronoi_cell_map.values():
        ...
        for edge, (A, U, tmin, tmax) in segment_list:
            ...
            coords += [shapely.Point(A + tmin * U)]
        
        coords += [shapely.Point(A + tmax * U)]
        polys += [shapely.Polygon(coords).intersection(area)]
